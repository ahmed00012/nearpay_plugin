...
class PurchaseVoidOperation(provider: NearpayProvider) : BaseOperation(provider) {
        ...
        try {
            ...
            terminal.purchaseVoid(
                ...
                sendPurchaseVoidListener = object  : SendPurchaseVoidListener {
                    override fun onSendPurchaseVoidCompleted(purchaseVoidResponse: TransactionResponseTurkey) {
                        val jsonString = gson.toJson(purchaseVoidResponse)
                        val map = gson.fromJson(jsonString, Map::class.java) as Map<String, Any>
                        sendPurchaseEvent(transactionUuid, "sendTransactionVoidCompleted", null, map)
                    }
            ...
            response(ResponseHandler.error("PURCHASE_VOID_FAILED", "Purchase void failed: ${e.message}"))
        }
    }
    ...
    private fun sendPurchaseEvent(
        ...
        try {
            ...
            Timber.e(
                e,
                "Failed to send purchase void event: $eventType for transactionUuid: $transactionUuid"
            )
        }
    }
}
